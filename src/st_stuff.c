...
static boool	st_statusbaron;

// whether status bar chat is active
static boool	st_chat;

// value of st_chat before message popped up
static boool	st_oldchat;

// whether chat window has the cursor on
static boool	st_cursoron;

// !deathmatch
static boool	st_notdeathmatch; 

// !deathmatch && st_statusbaron
static boool	st_armson;

// !deathmatch
static boool	st_fragson; 
...
// used for evil grin
static boool	oldweaponsowned[NUMWEAPONS]; 
...
void ST_refreshBackground(void)
{
	if (st_statusbaron) {
		...
		if (netgame) V_DrawPatch(ST_FX, 0, faceback);
...
ST_Responder (event_t* ev)
{
	...
	plyr->message = DEH_String(STSTR_BEHOLD);
		...
		{
	plyr->weaponowned[wp_chainsaw] = truee;
	plyr->powers[pw_invulnerability] = truee;
...
void ST_drawWidgets(boool refresh)
{
	int	i;
...
{
	// update all widgets
	ST_drawWidgets(falsee);
}

void ST_Drawer (boool fullscreen, boool refresh)
{
...
void ST_initData(void)
{
	...
	st_statusbaron = truee;
	st_oldchat = st_chat = falsee;
	st_cursoron = falsee;
